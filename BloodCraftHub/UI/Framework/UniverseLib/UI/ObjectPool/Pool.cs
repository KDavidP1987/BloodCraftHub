...
namespace BloodCraftHub.UI.Framework.UniverseLib.UI.ObjectPool;
...
public class Pool<T> : Pool where T : IPooledObject
{
    ...
    /// <summary>
    /// Optional default height for the object, necessary if this object can be used by a <see cref="ScrollPool{T}"/>.
    /// </summary>
    ...
    public void ReturnObject(T obj)
    {
        if (!borrowed.Contains(obj))
            LogUtils.LogWarning($"Returning an item to object pool ({typeof(T).Name}) but the item didn't exist in the borrowed list?");
        else
            borrowed.Remove(obj);
