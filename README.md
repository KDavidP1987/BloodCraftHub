...
**1 — Don't run BloodCraftHub and Eclipse together — use one or the other.**
BloodCraftHub gives you the same client-side readouts [Eclipse](https://thunderstore.io/c/v-rising/p/zfolmt/Eclipse/) does (XP / legacy / expertise / familiar / professions / quests / shift overlay), plus a lot more — it's meant to **replace** Eclipse on the client, not run alongside it. Running **both at once crashes the client on load**, and it's been re-confirmed against the latest Eclipse (1.3.14): the fault is inside Eclipse's own HUD code (`Eclipse.Services.CanvasService` reads a Unity `BufferLookup<ModifyUnitStatBuff_DOTS>` from a background coroutine — an access violation when BCH's normal activity invalidates that cached lookup; BCH itself is not in the crash stack). It can't be fixed from BCH's side. **Pick one: disable Eclipse in your mod manager while you use BloodCraftHub** (recommended, since BCH covers the same ground), or use Eclipse without BCH.
